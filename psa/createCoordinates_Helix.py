...
def createCoordinates_Helix(PS,T,l,Umax,offset,inpoFact,initialVector):
    ...
    for ind, val in enumerate(PS):
        ...
        radius = np.sin(np.arctan(Om/abs(offset)))                      #radius doesnt have an influence on curve because its proportional to height
        ...
        axisDir = chirality*B
        length = 2*np.pi*Om_s*radius*np.sqrt(1+(offset/Om)**2)*T/inpoFact
